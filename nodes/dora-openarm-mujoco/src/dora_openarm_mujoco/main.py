        ...
        for event in node:
            ...
            if eid in _ARM_INPUT_SIDES:
                ...
                if isinstance(value, pa.StructArray):
                    if "new_position" in value.type.names:
                        value = value.field("new_position")
                    elif "qpos" in value.type.names:
                        # Physical follower state: display the measured arm
                        # rather than the ideal command in digital-twin mode.
                        value = value.field("qpos")
                    else:
                        print(
                            f"[dora] Warning: unsupported struct fields "
                            f"{value.type.names} for {eid}"
                        )
                        continue
