#!/usr/bin/env python3
"""Guarded real right-arm EE +X 2 cm test using local OpenArm IK."""
...
EXPECTED_START = np.array(
    [0.19223, -0.06187, 1.11041, 0.05294, 0.66783, -0.06810, 0.73930]
)
...
def plan(qright: np.ndarray, qleft: np.ndarray):
    ...
    if np.linalg.norm(start[:3] - EXPECTED_START[:3]) > 0.01:
        raise RuntimeError(
            f"current EE differs from searched start by "
            f"{np.linalg.norm(start[:3]-EXPECTED_START[:3]):.4f} m"
        )
    target = start.copy()
    target[0] += 0.02
    ...
    for alpha in np.linspace(0.0, 1.0, STEPS + 1)[1:]:
        ...
        previous = result
    return np.asarray(trajectory), start, target, max_step
...
def main() -> int:
    ...
    try:
        ...
        if np.max(np.ptp(np.asarray(samples)[-5:], axis=0)) > 0.01:
            raise RuntimeError("unstable right-arm feedback")

        trajectory, start, target, max_step = plan(qright, qleft)
        ...
        if np.any(travel > 0.15):
            j = int(np.argmax(travel))
            raise RuntimeError(f"J{j+1} planned travel {travel[j]:.4f} rad exceeds 0.15")
        print("EE start :", np.array2string(start, precision=5), flush=True)
        print("EE target:", np.array2string(target, precision=5), flush=True)
        print("Final joint delta deg:", np.degrees(trajectory[-1] - qright), flush=True)
        ...
        def command(qcmd: np.ndarray, phase: str) -> None:
            ...
            if np.any(np.abs(q - previous) > 0.05):
                raise RuntimeError(f"{phase}: feedback jump")
        ...
        for qcmd in trajectory:
            command(qcmd, "forward")
        print("Reached right EE +X 2 cm target.", flush=True)
        for _ in range(100):
            command(trajectory[-1], "hold")
        ...
        print("SUCCESS: right EE forward 2 cm completed.", flush=True)
        return 0
    finally:
        if started:
            right.stop()
