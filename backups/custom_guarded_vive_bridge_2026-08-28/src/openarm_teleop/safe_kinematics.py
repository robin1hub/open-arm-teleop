...
class SafeKinematics(Kinematics):
    ...
    def set_max_joint_change_per_solve(self, max_change: float) -> None:
        self._require_ik().set_max_joint_change_per_solve(max_change)
...
class _StrictIKSolver:
    ...
    def set_max_joint_change_per_solve(self, max_change: float) -> None:
        if max_change <= 0.0:
            raise ValueError("max joint change must be positive")
        self._max_joint_change_per_solve = float(max_change)
