...
| VIVE Linux 纯仿真 | `./scripts/launch/run_vive_mujoco_sim.sh` | `YANGHAO_VIVE_BIMANUAL`：SteamVR 双手柄同时控制 OpenArm 1.0 双臂位置、方向和夹爪；IK 失败自动复位并继续跟随 |
| VIVE 双臂实体控制 | `./scripts/launch/run_vive_mujoco_real.sh --confirm-hardware` | `YANGHAO_VIVE_REAL`：显式 E 使能、P 失能同步；独立实机限速、关节限位、下一步碰撞预测和异常双臂失能 |
| VIVE 绝对位姿纯仿真 | `./scripts/launch/run_vive_absolute_mujoco_sim.sh` | `YANGHAO_VIVE_ABSOLUTE`：按 C 固定人体坐标标定，双手在绝对空间中的位置与方向映射到双臂；旧相对方案保留 |
| VIVE 共享绝对空间仿真 | `./scripts/launch/run_vive_shared_mujoco_sim.sh` | `YANGHAO_VIVE_SHARED`：使用手柄 tip 位姿和统一空间变换；双手柄重合时原始双臂目标重合 |
| VIVE 绝对位姿实体控制 | `./scripts/launch/run_vive_absolute_mujoco_real.sh --confirm-hardware` | `YANGHAO_VIVE_ABSOLUTE_REAL`：默认失能，C 标定、E 使能；连续投影、关节限位、躯干/双臂碰撞检查 |
| VIVE 共享绝对位姿实体控制 | `./scripts/launch/run_vive_shared_mujoco_real.sh --confirm-hardware` | `YANGHAO_VIVE_ABSOLUTE_REAL`：三步共享标定、默认失能；H 限速安全回位 |
