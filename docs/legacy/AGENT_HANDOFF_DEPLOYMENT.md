...
- `src/openarm_teleop/interactive_mujoco_ee_drag.py`: V1 model loading, joint resolver, FK helpers,
  physical/model conversion, collision checks and legacy mouse control.
- `src/openarm_teleop/safe_kinematics.py`: strict Mink differential IK that always retains
  configuration limits and anchors redundant solutions to the current posture.
- `src/openarm_teleop/vive_mujoco_teleop.py`: retained relative/clutched VIVE controller.
- `src/openarm_teleop/vive_absolute_mujoco_teleop.py`: absolute/shared calibration, OpenVR input,
  controller-tip mapping, bimanual target projection, gripper input and MuJoCo
  loop.
- `src/openarm_teleop/vive_mujoco_real_teleop.py`: `BimanualHardwareBridge`, feedback reads,
  enable/disable, joint velocity/acceleration limiting and tracking checks.
- `src/openarm_teleop/vive_absolute_mujoco_real_teleop.py`: physical subclass, E/P/H keys,
  physical soft limits, tracking-loss handling and command-shadow rendering.
- `config/openarm_safe_raw_zero.yaml`: active physical limits, CAN mapping, IDs and
  gains.
