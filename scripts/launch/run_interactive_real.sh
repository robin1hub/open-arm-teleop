...
echo "  command rate = 40 Hz; maximum joint step = 0.0087 rad (~20 deg/s)"
echo "  E = enable/disable; F = continuous follow; D = one-shot move"
echo "  native Control sliders = individual simulated joint targets"
echo "  double-click target, then Ctrl+mouse drag = native EE target control"
...
exec env PYTHONPATH="$project_dir/src${PYTHONPATH:+:$PYTHONPATH}" \
  .venv/bin/python -m openarm_teleop.interactive_mujoco_ee_drag \
  --model-version v1 \
  --real \
  --confirm-hardware \
