...
CONFIG = ROOT / "config" / "openarm_safe_raw_zero.yaml"

# The physical V1 configuration deliberately allows J4 to settle as low as
# -0.02 rad around the nominal MuJoCo zero.  Treat that small zero-position
# mismatch as alignment tolerance while continuing to reject larger offsets.
MJCF_ALIGNMENT_TOLERANCE = 0.02
...
def main() -> int:
    ...
    for side in ("right", "left"):
        ...
        below = ranges[:, 0] - joints
        above = joints - ranges[:, 1]
        violations = (below > MJCF_ALIGNMENT_TOLERANCE) | (
            above > MJCF_ALIGNMENT_TOLERANCE
        )
        tolerated = ((below > 0.0) | (above > 0.0)) & ~violations
        ...
        if np.any(tolerated):
            details = {
                f"J{index + 1}": float(max(below[index], above[index]))
                for index in np.flatnonzero(tolerated)
            }
            print(
                f"INFO: {side} nominal-zero MJCF boundary offsets tolerated "
                f"(limit {MJCF_ALIGNMENT_TOLERANCE:.3f} rad): {details}"
            )
